...
class PathFinder:
    ...
    def heuristic(self, x, y, x_t, y_t):
        dx = abs(x - x_t)
        dy = abs(y - y_t)
        dz = abs(self.terrainMap[y_t][x_t] - self.terrainMap[y][x])
        return 10 * (dx + dy) - 4 * (min(dx, dy)) + dz
    ...
    def BreadthFirst(self, target):
        ...
        while len(q) is not 0:
            ...
            for i in self.neighbours:
                ...
                if self.legalMove(x_n, y_n) and not visited[y_n][x_n] and self.shouldMove(x, y, x_n, y_n):
